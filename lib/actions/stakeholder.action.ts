...
export async function createStakeholder(params: CreateStakeholderParams) {
	try {
		...
		if (existingStakeholder) {
			throw new Error(
				'Stakeholder with this email already exists for this event'
			);
		}
 ...
 */
export async function getEventStakeholders(
	eventId: string,
	filters?: {
		role?: string;
...
export async function bulkImportStakeholders(
	params: BulkImportStakeholdersParams
) {
	try {
		await connectToDatabase();
 ...
 */
async function processStakeholderFile(
	importId: string,
	params: BulkImportStakeholdersParams
) {
	try {
		...
		const stats = await Stakeholder.aggregate([
			{ $match: { event: new mongoose.Types.ObjectId(eventId) } },
			{
				$group: {
					...
					registeredCount: {
						$sum: {
							$cond: [{ $eq: ['$attendanceStatus', 'registered'] }, 1, 0],
						},
					},
					cancelledCount: {
						$sum: {
							$cond: [{ $eq: ['$attendanceStatus', 'cancelled'] }, 1, 0],
						},
					},
				},
			},
