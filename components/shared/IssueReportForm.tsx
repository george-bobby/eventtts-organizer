...
export default function IssueReportForm({
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    const newAttachments = Array.from(files).map(
      (file) => URL.createObjectURL(file), // This is just a placeholder
    );
  ...
  return (
    <Card
      className={isModal ? "w-full border-0 shadow-none" : "max-w-2xl mx-auto"}
    >
      <CardHeader className="pb-4 sm:pb-6">
        ...
        <div className="bg-gray-50 p-3 sm:p-4 rounded-lg border mt-4">
          <div className="text-xs sm:text-sm text-gray-600 space-y-1">
            ...
            <div>
              <strong>Organizer:</strong> {(event.organizer as any)?.firstName}{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          {formData.category && (
            <div className="space-y-2 sm:space-y-3">
              ...
              >
                ...
                <SelectContent>
                  {issueCategories[
                    formData.category as keyof typeof issueCategories
          ...
          <div className="space-y-2 sm:space-y-3">
            ...
            >
              {severityOptions.map((option) => (
                <div
                  key={option.value}
                  className={`flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 rounded-lg border transition-colors ${formData.severity === option.value ? option.color : "hover:bg-gray-50"}`}
                >
                  <RadioGroupItem
                    value={option.value}
                    id={option.value}
          ...
          {showAdvanced && (
            <div className="space-y-3 sm:space-y-4 border-t pt-3 sm:pt-4">
              ...
              <div className="space-y-2">
                <Label className="text-sm sm:text-base font-medium">
                  Attachments (Screenshots, Photos, etc.)
                </Label>
